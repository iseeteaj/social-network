...
describe('Users', function(){
    ...
    //We create a pretend request object, with a user object attached to req.session (we pretend we have a session).//

    //We mock res and next objects that our controller expects. //
    //Our controller will actually call next() as a callback, as it will be used as middleware. //
    //Therefore we assign next a mock function //
    //(in previous walkthroughs we've passed done to our controllers - //
    //we don't need to here as our logout controller won't perform any asyncronous database operations).
        ...
        test('logout clears the session', function () {
            ...
            //We then call a logout method on usersControllers, passing in our req, res and next.//
            usersControllers.logout(req, res, next)

            //Then we expect req.session.user to be a falsy value (we expect it to have been set to nothing).//
